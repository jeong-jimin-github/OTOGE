...
public class NoteGen : MonoBehaviour
{
    ...
    private void Load(string SongName)
    {
        ...
        for (int i = 0; i < inputJson.notes.Length; i++)
        {
            ...
            float time = (beatSec * inputJson.notes[i].num / (float)inputJson.notes[i].LPB) + inputJson.offset / 10000;
            ...
            else if (inputJson.notes[i].type == 2) // 롱노트 생성
            {
                ...
                {
                    float timea = (beatSec * inputJson.notes[i].notes[a].num / (float)inputJson.notes[i].notes[a].LPB) + inputJson.offset / 10000;
